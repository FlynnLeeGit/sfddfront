<template>
  <div>
    ...
    <section class="feature"
             container>
      <h1 v-lazy.animate="'slideInUp'">中孚空间 · 预案式成品整装</h1>
      <h3 v-lazy.animate="'slideInUp'">已为您规划好完整的设计预案，从硬装到软装的造价清单，从制图到全景展示，预案中一应俱全。<br>帮你把关选材搭配，预算控制，氛围意境。选择我们的当季预案，<br>来匹配您的房型，设计费仅以2折收取</h3>
    </section>
    ...
    <section class="beauty"
      ...
      <div col
           class='beauty__desc'>
        <h1 v-lazy.animate="'slideInUp'">美感 · 低价 · 高质</h1>
        <p v-lazy.animate="'slideInUp'">每季度，我们的设计董事带领总监级设计师会推出当季的整装预案，
    ...
    <section class="standard -design"
      ...
      <div class="standard__content">
        <h2 v-lazy.animate="'slideInUp'">整装设计标准</h2>
        <p v-lazy.animate="'slideInUp'">每套整装方案中，我们充分考虑了十二类家具收纳、十条家居动线、六类色彩搭配、核心风格与主打元素。我们基于每个风格，我们会主导三个近似的搭配，并提供软装包（可拆项选择）。帮助您第一时间明确造价、材料、施工内容、设备与软装。</p>
        <nuxt-link v-lazy.animate="'slideInUp'"
                   class="Btn -black"
    ...
    <section class="standard -quality"
      ...
      <div class="standard__mask"></div>
      <div class="standard__content">
        <h2 v-lazy.animate="'slideInUp'">质量控制标准</h2>
        <p v-lazy.animate="'slideInUp'">我们作为专业的项目总承包商，深谙质量责任之重大，在以下六件事上不敢有半点马虎。自有施工班组成熟稳定、严审制图（深化）精度、先放线后施工、行业领先的工艺要求、稳定成熟的建材配套与尽心尽责的质检管理；并通过我们的协同管理系统，各司其职，统一汇报，为用户提供远超国标的质量标准。</p>
        <nuxt-link v-lazy.animate="'slideInUp'"
                   class="Btn -black"
    ...
    <section class="standard -material"
      ...
      <div class="standard__mask"></div>
      <div class="standard__content">
        <h2 v-lazy.animate="'slideInUp'">材料选择标准</h2>
        <p v-lazy.animate="'slideInUp'">我们拥有强大的供应链跟严格的供应链考评机制，精选符合设计的国内外一线品牌，从质量稳定、环保、价格、配送及时、安装合规、产能稳定这七个维度，以季度为单位进行评测。采用集采预定的方式对供应商下单，确保我们的采购成本优势能体现在整装预案之内。</p>
        <nuxt-link v-lazy.animate="'slideInUp'"
                   class="Btn -black"
    ...
    <section class="service">
      <div container>
        <h1 v-lazy.animate="'fade'">中孚空间服务</h1>
        <ul class="service__list"
            grid>
          <nuxt-link v-lazy.animate="'slideInUp '+ 'delay' + idx * 200"
                     tag='li'
                     class="service__item"
                     :key='s.icon'
                     :to='s.to'
